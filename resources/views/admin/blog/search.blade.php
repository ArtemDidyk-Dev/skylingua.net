                ...
                <div class="card-body">
                    ...
                    <table class="table table-hover table-striped" data-sorting="true">
                        ...
                        @foreach($blogs as $blog)
                            <tr class="table-id-{{ $blog->id }}" data-index="{{ $blog->id }}" data-position="{{ $blog->sort }}">
                               <!-- ID -->
                                <td>{{$blog->id}}</td>

                                <!--  NAME  -->
                                <td><a href="{{ route('admin.blog.edit',$blog->id) }}">{{ $blog->name }}</a></td>

                                <!--  Tarix  -->
                                <td>{{ \Illuminate\Support\Carbon::parse($blog->updated_at)->format('Y-m-d H:i') }}</td>
                                        ...
                                        <div class="dropdown-menu p-0 m-0 dropdown-menu-md dropdown-menu-right"
                                            ...
                                            <ul class="navi navi-hover">
                                                ...
                                                <li
                                                    ...
                                                    <a href="{{ route('admin.blog.edit',$blog->id) }}" class="navi-link text-center">
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                        <div class="d-flex align-items-center py-3">
                            <span class="text-muted">
                                Total <b><span class="totalCount">{{ $blogs->total() }}</span></b> item
                                @if($blogs->hasPages())
                                    , <b>{{ $blogs->lastPage() }}
                                </b> blogdan  <b>{{ $blogs->currentPage() }}</b>-ci göstərildi.
                                @endif
    ...
    <script>
        $(function () {
            ...
            $(document).on('click', '.statusActive', function () {
                ...
                $.ajax({
                    url: "{{ route('admin.blog.statusAjax') }}",
                    type: 'POST',
        ...
        $(document).on('click', '.deleteButton', function () {
            ...
            $.ajax({
                url: "{{ route('admin.blog.deleteAjax') }}",
                type: 'POST',
                data: {id: dataID},
                dataType: 'JSON',
                success: function (response) {
                    if (response.success) {
                        ...
                        Swal.fire({
                            title: "Attention?",
                            html: "Blogı silmək istədiyinizə əminsiniz?",
                            icon: "error",
