            ...
            <div class="d-flex align-items-center flex-wrap mr-1">
                ...
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        ...
                        @isset($searchText)
                            <li class="breadcrumb-item">
                                <a href="{{ route('admin.language.index') }}" class="text-muted">Notifications</a>
                            </li>
                            <li class="breadcrumb-item">
                                Search
                            </li>
                ...
                <div class="card-body">
                    ...
                    <table class="table table-hover table-striped" data-sorting="true">
                        ...
                        @foreach($notifications as $notification)
                            <tr class="table-id-{{ $notification->notification_id }}" data-index="{{ $notification->notification_id }}" data-position="{{ $notification->sort }}">
                                <!-- ID -->
                                <td>{{$notification->notification_id}}</td>
                                <!--  NAME  -->
                                <td>{{ $notification->notificationsTranlations[0]->text }}</td>
                                <td>{{ $notification->getUser->name }} ({{ $notification->getUser->email }})</td>

                                <!--  Tarix  -->
                                <td>{{ updateDate($notification->updated_at,$notification->notificationsTranlations) }}</td>
                                ...
                                <td width="100">
                                        ...
                                        <label>
                                            <input
                                                ...
                                                type="checkbox"
                                                {{ $notification->notification_status == 1? 'checked="checked"':"" }}
                                                name="select">
                                            <span></span>
                                        </label>
									</span>
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                        <div class="d-flex align-items-center py-3">
                            <span class="text-muted">
                                Total <b><span class="totalCount">{{ $notifications->total() }}</span></b> items
                                @if($notifications->hasPages())
                                    , <b>{{ $notifications->lastPage() }}
                                </b> səhifədən  <b>{{ $notifications->currentPage() }}</b>-ci göstərildi.
                                @endif
    ...
    <script>
        $(function () {
            ...
            $(document).on('click', '.statusActive', function () {
                ...
                $.ajax({
                    url: "{{ route('admin.notification.statusAjax') }}",
                    type: 'POST',
        ...
        $(document).on('click', '.deleteButton', function () {
            ...
            $.ajax({
                url: "{{ route('admin.notification.deleteAjax') }}",
                type: 'POST',
                data: {id: dataID},
                dataType: 'JSON',
                success: function (response) {
                    if (response.success) {
                        ...
                        Swal.fire({
                            title: "Attention?",
                            html: "Are you sure you want to delete the notification?",
                            icon: "error",
